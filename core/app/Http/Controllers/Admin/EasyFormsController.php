...
class EasyFormsController extends Controller
{
    ...
    public function easyForms()
    {
        $easyForms  = new EasyForm([
            "easy_form_server_url"  => '',
            "easy_form_digital"     => '',
            "easy_form_restricted"  => '',
        ]);

        if(EasyForm::count() >= 1) $easyForms   = EasyForm::first();

        return view('admin.easyforms.index', compact('easyForms'));
    }

    public function updateEasyForms(Request $request)
    {
        $data['easy_form_server_url']   = $this->helper_clean_base_url($request->easy_form_server_url);
        $data['easy_form_digital']      = trim($request->easy_form_digital);
        $data['easy_form_restricted']   = trim($request->easy_form_restricted);

        $this->forms_url                = $data['easy_form_server_url'];

        EasyForm::firstOrCreate([])->update($data);


        return redirect()
        ->route('admin.easy-forms')
        ->with('success', 'Easy-Forms updated successfully');
    }

    public function helper_clean_base_url(string $url)
    {
        return Str::endsWith(trim($url), '/') ? trim(substr($url, 0, strlen(trim($url))-1)) : trim($url);
    }
    public function helper_parse_form($form)
    {
        //return $form;
        //$payload        = $form;
        ...
        {
            return  $this->make_abs_url($link->outerHtml(), "href");
        });
        ...
        {
            return  $this->make_abs_url($script->outerHtml(), "src");
        });
        ...
        return [
            ...
            "styles"    => [
                "tags"  => "<style>".$style_tags->eq(0)->html("")." body{padding:0px !important}</style>",
                "links" => $style_links,
            ],
            "scripts"   => [
