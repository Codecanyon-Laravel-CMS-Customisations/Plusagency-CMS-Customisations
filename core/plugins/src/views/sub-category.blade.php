...
@section('content')
  <div class="page-header">
    <h4 class="page-title">Product Sub-Child Categories</h4>
                            ...
                            <div class="btn-group">
                                @if($category->show_in_menu == 1)
                                    ...
                                    <div class="dropdown-menu">
                                        <a href="{{route('plugins.child-category.edit', $category->id) . '?language=' . request()->input('language')}}" class="dropdown-item">
                                ...
                                @endif
                                <form id="addtomenuform-{{$category->id}}" class="deleteform d-none" action="{{route('plugins.child-category.toggle_show_in_menu', $category->id)}}" method="post">
                                    @csrf
                                    <input type="hidden" name="category_id" value="{{$category->id}}">
                                </form>
                                <form id="deleteform-{{$category->id}}" class="deleteform d-none" action="{{route('plugins.child-category.destroy')}}" method="post">
                                    @csrf
                                    <input type="hidden" name="category_id" value="{{$category->id}}">
                                </form>
                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="ajaxForm" class="modal-form" action="{{route('plugins.child-category.store')}}" method="POST">
            @csrf
