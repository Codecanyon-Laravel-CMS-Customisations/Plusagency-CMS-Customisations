    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          ...
          <a class="btn btn-info btn-sm float-right d-inline-block" href="{{route('plugins.category.index') . '?language=' . request()->input('language')}}">
        ...
        <div class="card-body pt-5 pb-5">
          <div class="row">
            <div class="col-lg-6 offset-lg-3">
              <form id="ajaxForm"  action="{{route('plugins.category.update')}}" method="POST">
                @csrf
                <div class="form-group">
                    <label for="">Parent Category **</label>
                    <select name="parent_menu_id" class="form-control">
                        @foreach ($parents as $parent => $id)
                            <option @if($data->parent_category)
                                {{$data->parent_category->id == $id ? 'selected' : ''}}
                            @endif value="{{ $id }}">{{ $parent }}</option>
                        @endforeach
                    </select>
