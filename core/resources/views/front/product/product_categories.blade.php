                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                @foreach ($pc1->childs->sortBy('name', 0, false) as $pc2)
                                    <p class="card-text">
                                        <a
                                            href="/products?search=&sc-id={{ $pc2->id }}&type=new">{{ $pc2->name }}</a>
                                    </p>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="accordion" id="productCategoriesAccordion">
                        @foreach ($pcategories->where('menu_level', '1') as $pc1)
                            ...
                            <div class="card">
                                <div class="card-header" id="heading{{ $pc1->id }}">
                                    <h5 class="card-title">
                                        <a class="" data-toggle="collapse"
                                            data-target="#collapse{{ $pc1->id }}"
                                            aria-expanded="@if ($loop->first) true @else false @endif"
                                            aria-controls="collapse{{ $pc1->id }}">
                                            {{ $pc1->name }}
                                            <i class="fas fa-angle-right"></i>
                                        </a>
                                    </h5>
                                ...
                                <div id="collapse{{ $pc1->id }}" class="collapse @if ($loop->first) show @endif"
                                    aria-labelledby="heading{{ $pc1->id }}" data-parent="#productCategoriesAccordion">
                                    <div class="card-body">
                                        @foreach ($pc1->childs->sortBy('name', 0, false) as $pc2)
                                            <p class="card-text">
                                                <a
