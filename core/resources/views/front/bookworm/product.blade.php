                ...
                <div class="shop-tab-area">
                    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                        ...
                        @if ( !$pvariation )
                            ...
                            <li class="nav-item">
                                <a class="nav-link" id="pills-attributes-tab" data-toggle="pill" href="#pills-attributes" role="tab" aria-controls="pills-attributes" aria-selected="false">Details</a>
                            </li>
                            @endif
                        @else
                        ...
                        @if ( ! is_null( $be->product_tabs ) )
                            ...
                            @endphp
                            <li class="nav-item">
                                <a class="nav-link" id="pills-{{ Str::slug( $tab->title ) }}-tab" @if($tab->type == 'content') data-toggle="pill" @endif href="{{ $tab->type == 'content' ? $tab_link : $tab->link }}" role="tab" aria-controls="pills-{{ Str::slug( $tab->title ) }}" aria-selected="false" @if($tab->content == 'link') target="_blank" @endif>{{ $tab->title }}</a>
                            </li>
                            @endforeach
                        @endif
...
{{--                                {!! replaceBaseUrl(convertUtf8(str_replace('\n', '', nl2br($product->description, false)))) !!}--}}
                        ...
                        @if ( ! is_null( $be->product_tabs ) )
                            @foreach ( json_decode( $be->product_tabs ) as $tab )
                                ...
                                @if ( $tab->global == '0' )
                                    @if ($tab->product_id != $product->id) @continue @endif
                                @endif
                                <div class="tab-pane fade show" id="pills-{{ Str::slug( $tab->title ) }}" role="tabpanel" aria-labelledby="pills-home-tab">
                        ...
                        <div class="tab-pane fade show" id="pills-attributes" role="tabpanel" aria-labelledby="pills-home-tab">
                            @if ( ! is_null( $be->product_attributes ) )
                            <ul class="list-group">
                                @if ( ! is_null( $be->product_attributes ) )
                                    @foreach( json_decode($be->product_attributes) as $attribute )
                                        <li class="list-group-item"><strong>{{ $attribute->name }}</strong> - {{ $attribute->value }}</li>
                                    @endforeach
                                @endif
                            </ul>
                            @endif
                            ...
                            <ul class="list-group">
                                ...
                                @if ( ! is_null( $product->custom_fields ) )
                                    @foreach( json_decode($product->custom_fields) as $field )
                                        <li class="list-group-item"><strong>{{ $field->name }}</strong> - {{ $field->value }}</li>
                                    @endforeach
                                @endif
                                @if($pvariation)
