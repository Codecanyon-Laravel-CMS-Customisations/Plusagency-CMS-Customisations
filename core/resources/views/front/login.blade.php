                ...
                <div class="col-lg-8">
                    @if ($bex->product_guest_checkout == 1 && !empty(request()->input('redirected')) && request()->input('redirected') == 'checkout' && !containsDigitalItemsInCart())
                        <a href="{{ route('front.checkout', ['type' => 'guest']) }}"
                            class="btn btn-block btn-primary mb-4 base-bg py-3 border-0">{{ __('Checkout as Guest') }}</a>

                        <div class="mt-4 mb-3 text-center">
                            <h3 class="mb-0"><strong>{{ __('OR') }},</strong></h3>
                        </div>
                    @elseif($bex->package_guest_checkout == 1 && !empty(request()->input('redirected')) &&
                        request()->input('redirected') == 'package-checkout')
                        <a href="{{ session()->get('link') . '?type=guest' }}"
                            class="btn btn-block btn-primary mb-4 base-bg py-3 border-0">{{ __('Checkout as Guest') }}</a>
                        ...
                        <form id="loginForm" action="{{ route('user.login') }}" method="POST">
                            ...
                            <div class="input-btn">
                                ...
                                <p class="float-lg-right float-left">{{ __("Don't have an account ?") }} <a
                                    ...
                                    {{ __('to create one.') }}</p>
                                <a class="mr-3"
                                    href="{{ route('user-forgot') }}">{{ __('Lost your password?') }}</a>
                            </div>
                        </form>
                    </div>
                </div>
